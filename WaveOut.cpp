...
void normalizeAudio(std::vector<float> &data, float TargetMaxVal_dB)
{
    ...
    {
        scale = 1.f / max_val;
        float max_val_dB_lin = powf(10.f, TargetMaxVal_dB / 20.f);
...
void normalizeAudio(juce::AudioBuffer<float> &data, float TargetMaxVal_dB)
{
    ...
    float max_val = 0.f;
    for (int cc = 0; cc < numchannels; cc++)
    {
        for (int ss = 0; ss < numsamples; ss++)
        {
            float abs_d = std::fabs(dataptr[cc][ss]);
            if (abs_d > max_val)
                max_val = abs_d;
        }
    }
